...
### 캘린더 위젯 + 데이터 점 표시 ✨
![캘린더 위젯](screenshots/phase5-calendar.png)
![캘린더 데이터 점](screenshots/phase6-calendar-dots.png)

**주요 기능:**
- 날짜 클릭 시 캘린더 팝업 표시
- 월 선택 UI (◀ 2026년 1월 ▶)
- 날짜 그리드 (일~토, 요일별 색상)
- 오늘 날짜 하이라이트 (연한 파란색 배경)
- 선택된 날짜 강조 (진한 파란색)
- 월 이동 화살표 (이전/다음 월)
- "오늘" 버튼으로 빠른 이동
- 외부 영역 클릭 또는 취소 버튼으로 닫기
- **데이터 점 표시** ✅:
  - 🔵 파란 점: 계획만 있는 날 (14일, 16일, 20일)
  - 🟢 초록 점: 실행만 있는 날 (18일, 22일)
  - 🔵🟢 2색 점: 계획+실행 모두 있는 날 (13일, 15일)
...
#### Phase 5: 캘린더 위젯
- **캘린더 팝업**: 날짜 버튼 클릭 시 모달 캘린더 표시
- **월별 네비게이션**: 이전/다음 월 이동 (◀ 2026년 1월 ▶)
- **날짜 그리드**: 일~토 요일별 색상 구분 (일요일: 빨강, 토요일: 파랑)
- **날짜 선택**: 클릭 시 타임라인 변경 + 캘린더 자동 닫힘
- **오늘 하이라이트**: 오늘 날짜 연한 파란색 배경 표시
- **선택된 날짜 강조**: 현재 선택 날짜 진한 파란색 배경
- **빠른 이동**: "오늘" 버튼으로 현재 날짜 이동
- **외부 클릭 닫기**: 캘린더 외부 영역 클릭 또는 취소 버튼
- **월별 데이터 집계**: 계획/실행 데이터 존재 여부 추적

#### Phase 6: 캘린더 데이터 점 표시 ✨ NEW
- **자동 데이터 집계**: `useCalendarData` 훅으로 월별 모든 날짜 데이터 추적
- **3가지 점 스타일**:
  - 🔵 파란 점 (1.5px): 계획만 있는 날
  - 🟢 초록 점 (1.5px): 실행만 있는 날
  - 🔵🟢 2색 점 (각 1px): 계획+실행 모두 있는 날
- **날짜별 표시**: 각 날짜 하단 중앙에 점 표시
- **실시간 업데이트**: 월 변경 시 자동으로 데이터 재집계
